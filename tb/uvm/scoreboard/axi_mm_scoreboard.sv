...
// Create 4 different analysis_imp types: p0 / p1 / commit / apply
...
class axi_mm_scoreboard #(
    ...
    // -------------------------------------------------------------------------
    // Robust processing: FIFOs per port + commit/apply fifo
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Memory model (byte-addressed) driven by APPLY visibility
    // -------------------------------------------------------------------------
    ...
    // So all beats in the same apply burst must share the first-beat visibility time.
    // Also, while apply emission is still in progress, reads that touch the not-yet-
    // emitted future tail of this burst must be deferred.
    // -------------------------------------------------------------------------
    typedef struct {
        bit                    valid;
    ...
    // extra deferred statistics (STAT ONLY, no behavior change)
    ...
    // -------------------------------------------------------------------------
    // COMMIT apply scheduler (kept for minimal structural impact)
    // -------------------------------------------------------------------------
    typedef struct {
        time                          t_apply;
    ...
    // -------------------------------------------------------------------------
    // Commit queue sorted insert (kept for minimal structural impact)
    // -------------------------------------------------------------------------
    ...
    // apply: DUT visibility is burst-atomic, so all beats in the same apply burst
    // share the first apply beat time as common visibility time.
    task automatic drain_apply_fifo();
        ...
        while (fifo_apply.try_get(ait)) begin
            ...
            if (!size_is_legal(ait.size)) begin
                `uvm_error("SCB", $sformatf(
                    "Illegal apply size=%0d (>log2(%0d)) port=%0d id=0x%0h beat_idx=%0d addr=0x%0h",
                    ait.size, BYTES_PER_BEAT, p, ait.id, ait.beat_idx, ait.byte_addr))
            end
            ...
            if ((ait.beat_idx == '0) ||
                ...
                if (inflight_apply_burst[p].linear_incr_like) begin
                    if ((cur_bytes != inflight_apply_burst[p].beat_bytes) ||
                        (ait.byte_addr != (inflight_apply_burst[p].last_addr +
                                           inflight_apply_burst[p].beat_bytes))) begin
                        inflight_apply_burst[p].linear_incr_like = 1'b0;
                    end
                end
    ...
    function automatic bit read_hits_any_inflight_apply_future_tail(
        ...
        for (int p = 0; p < 2; p++) begin
            ...
            if (!st.linear_incr_like)
                return 1'b1; // conservative defer for non-linear / ambiguous bursts

            emitted_end_addr = st.last_addr + st.beat_bytes - 1;

            // overlap with the not-yet-emitted future tail of the current atomic apply burst
        ...
        for (int i = 0; i < beats; i++) begin
            ...
            if (!beat_compare_ok(beat_addr, tr.size, expected, got)) begin
                ...
                for (int b = 0; b < bytes; b++) begin
                    ...
                    `uvm_info("SCB_DBG",
                        $sformatf("  byte[%0d] lane=%0d cur_addr=0x%0h mem_idx=%0d exp_byte=0x%02h got_byte=0x%02h written_as_of=%0b last_commit_as_of=%0t hist_val=0x%02h",
                                b, lane, cur_addr, base_off,
                                expected[8*lane +: 8], got[8*lane +: 8],
                                hist_byte_written_as_of(base_off, beat_t),
                                hist_t, hist_val),
                        UVM_NONE)
                end
            end
    ...
    task automatic enqueue_read_for_retry(
        ...
        `uvm_info("SCB",
            $sformatf("Defer read compare: port=%0d id=0x%0h addr=0x%0h beats=%0d pending_read_q=%0d",
                      src_port, tr.id, tr.addr, tr.len+1, pending_read_q.size()),
            UVM_HIGH)
    endtask
    ...
    task automatic handle_read(
        ...
        if (tr.rtime_beats.size() != (tr.len + 1) || tr.rdata_beats.size() != (tr.len + 1)) begin
            `uvm_warning("SCB",
                $sformatf("READ compare SKIPPED: missing/invalid beat arrays (id=0x%0h port=%0d beats=%0d rtime=%0d rdata=%0d).",
                          tr.id, src_port, (tr.len+1), tr.rtime_beats.size(), tr.rdata_beats.size()))
        ...
        if (read_fully_ready_to_compare(tr, src_port)) begin
            ...
            for (int i = 0; i < beats; i++) begin
                ...
                if (read_hits_any_inflight_apply_future_tail(beat_addr, tr.size, beat_t)) begin
                    `uvm_info("SCB",
                        $sformatf("Skip read compare: apply-future-tail beat_addr=0x%0h beat=%0d port=%0d id=0x%0h beat_t=%0t",
                                  beat_addr, i, src_port, tr.id, beat_t),
                        UVM_HIGH)
                    break;
                end
                ...
                if (!beat_is_fully_stable_written(src_port, beat_addr, tr.size, beat_t)) begin
                    `uvm_info("SCB",
                        $sformatf("Skip read compare: not-applied/unstable beat_addr=0x%0h beat=%0d port=%0d id=0x%0h (stable_delay=%0t beat_t=%0t)",
    ...
    task automatic retry_pending_reads();
        ...
        while (pending_read_q.size() > 0) begin
            ...
            if (read_fully_ready_to_compare(cur.tr, cur.src_port)) begin
                ...
                reads_retried_ok++;
                `uvm_info("SCB",
                    $sformatf("Deferred read compare completed: port=%0d id=0x%0h addr=0x%0h retries=%0d",
                              cur.src_port, cur.tr.id, cur.tr.addr, cur.retry_count),
                    UVM_HIGH)
            end
            else begin
                cur.retry_count++;
    ...
    task run_phase(uvm_phase phase);
        ...
        `uvm_info("SCB", $sformatf("Scoreboard started (STRICT_RANGE=%0d) [APPLY-driven visibility + deferred read compare] stable_delay=%0t",
        ...
        // give retry path a little time to settle after objections drop
        repeat (20) begin
            drain_apply_fifo();
            drain_commit_fifo();
            retry_pending_reads();
    ...
    function void report_phase(uvm_phase phase);
        ...
        // stats only: final categorization of remaining pending reads
        compute_pending_read_final_stats();

        `uvm_info("SCB",
                  $sformatf("FINAL stats: writes(p0=%0d p1=%0d) commit_beats(p0=%0d p1=%0d) reads(p0=%0d p1=%0d) mismatches=%0d deferred=%0d retried_ok=%0d pending_read=%0d pending_commit=%0d",
                            writes_seen_p0, writes_seen_p1,
                            commit_beats_seen_p0, commit_beats_seen_p1,
                            reads_seen_p0, reads_seen_p1,
                            mismatches,
                            reads_deferred, reads_retried_ok,
                            pending_read_q.size(), pending_commit_q.size()),
                  UVM_LOW)

        if (pending_read_q.size() != 0) begin
            `uvm_warning("SCB",
                $sformatf("FINAL: %0d deferred read(s) still pending compare at end of test", pending_read_q.size()))

            `uvm_info("SCB",
                $sformatf("FINAL deferred breakdown: p0=%0d p1=%0d | retry_count: 0=%0d 1=%0d 2_3=%0d 4_7=%0d 8_plus=%0d | age: <100ns=%0d 100ns_1us=%0d 1us_10us=%0d >=10us=%0d",
                          ...
                          pending_read_age_1us_10us, pending_read_age_ge_10us),
                UVM_LOW)

            `uvm_info("SCB",
                $sformatf("FINAL deferred reason: no_written=%0d unstable_only=%0d mixed=%0d other=%0d",
                          pending_reason_no_written_byte,
                          pending_reason_only_unstable_byte,
                          pending_reason_mixed_no_written_and_unstable,
                          pending_reason_other),
            ...
            if (pending_first_blocked_beat_samples != 0) begin
                `uvm_info("SCB",
                    $sformatf("FINAL deferred first_bad_beat: min=%0d max=%0d avg=%0f samples=%0d",
