...
class axi_mm_monitor #(
    ...
    localparam time         IGNORE_WINDOW     = 500ns; // tune: 200ns~2us based on the fabric
    localparam time         DUP_B_DROP_WINDOW = 5us;   // tune: Worst-case response latency
    ...
    // Outstanding tables
    aw_tr_t write_q[$];
    aw_tr_t wait_b_s[int unsigned];          
    ar_tr_t pending_reads_s[int unsigned];  

    // Last completion timestamp per ID
    time last_wr_done_time[int unsigned];    
    uvm_analysis_port #(axi_mm_seq_item#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH)) ap;

    // Global reset/flush events
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db#(virtual axi_mm_if#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH, 1).mp_monitor)::get(this, "", "vif_mon", vif))
        begin
            if (!uvm_config_db#(virtual axi_mm_if#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH, 1).mp_monitor)::get(this, "", "vif", vif))
            begin
                `uvm_fatal("NOVIF", "Monitor mp_monitor not set")
            end
        end
        ...
        `uvm_info("VIF", $sformatf("mp_monitor=%p", vif), UVM_LOW)
    ...
    // ------------------------------------------------------------
    // Centralized clear for reset/flush
    // ------------------------------------------------------------
    task automatic clear_state(string why);
        ...
        `uvm_info("MON_CLR", $sformatf("State cleared %s, ignore_unknown_until=%0t reset_active=%0b", why, ignore_unknown_until, reset_active), UVM_LOW)
    endtask
    ...
    task reset_assert_watchdog();
        forever begin
            ...
            `uvm_info("MON_RST", "Reset asserted -> cleared state", UVM_LOW)
        end
    endtask
    ...
    task reset_deassert_watchdog();
        forever begin
            ...
            reset_active = 1'b0;
            `uvm_info("MON_RST", "Reset deasserted", UVM_LOW)
        end
    endtask
    ...
    task flush_watchdog();
        forever begin
            ...
            #0; // Avoid missed-trigger race
            ev_flush_done.trigger();
            `uvm_info("MON_FLUSH", "Flushed -> trigger flush_done", UVM_LOW)
        end
    endtask
        ...
        unique case (burst)
            ...
            2'b10: begin                                            // WRAP
                if ((wrap_bytes & (wrap_bytes - 1)) != 0) begin
                    `uvm_error("MON", $sformatf("Illegal WRAP: wrap_bytes=%0d not power-of-2 (start=0x%0h len=%0d size=%0d)", wrap_bytes, start_addr, len, size))
                    return start_addr;
                end
    ...
    // ------------------------------------------------------------
    // Helper function: Find outstanding write by ID
    // ------------------------------------------------------------
    ...
    task automatic complete_and_emit(ref aw_tr_t e);
        ...
        `uvm_info("MON_WR_DONE", $sformatf("WRITE completed: ID=%0d addr=0x%0h beats=%0d bresp=%0d done_time=%0t", int'(e.tr.id), e.tr.addr, expected_beats, e.tr.bresp, e.tr.done_time), UVM_LOW)
        ...
        forever begin
            ...
            if ((vif.cb_monitor.awvalid === 1'b1) && (vif.cb_monitor.awready === 1'b0)) begin
                ...
                if ((aw_wait_cyc_w % 100) == 0) begin
                    `uvm_info("MON_AW_WAIT", $sformatf("AW waiting %0d cyc, v=%0b r=%0b addr=0x%0h id=%0d len=%0d burst=%02b size=%0d", aw_wait_cyc_w, vif.cb_monitor.awvalid, vif.cb_monitor.awready, vif.cb_monitor.awaddr,  vif.cb_monitor.awid, vif.cb_monitor.awlen,   vif.cb_monitor.awburst, vif.cb_monitor.awsize), UVM_LOW)
                end
            end else begin
                aw_wait_cyc_w = 0;
            end
            ...
            if ((vif.cb_monitor.awvalid === 1'b1) && (vif.cb_monitor.awready === 1'b1)) begin
                ...
                tr_struct.tr = axi_mm_seq_item#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH)::type_id::create($sformatf("wr_tr_id_%0d", int'(vif.cb_monitor.awid)), this);
                ...
                `uvm_info("MON_AW_HS", $sformatf("AW HS: ID=%0d addr=0x%0h len=%0d burst=%02b size=%0d q_depth=%0d", int'(tr_struct.tr.id), tr_struct.tr.addr, tr_struct.tr.len, tr_struct.tr.burst, tr_struct.tr.size, q_depth_after), UVM_LOW)
            end
            ...
            if ((vif.cb_monitor.wvalid === 1'b1) && (vif.cb_monitor.wready === 1'b1)) begin
                ...
                if (write_q.size() == 0) begin
                    state_lock.put(1);
                    `uvm_error("MON", $sformatf("W HS but write_q empty, WDATA=0x%0h WSTRB=0x%0h WLAST=%0b", vif.cb_monitor.wdata, vif.cb_monitor.wstrb, vif.cb_monitor.wlast))
                end else begin
                    ...
                    if (beat_idx >= expected_beats) begin
                        state_lock.put(1);
                        `uvm_error("MON",
                            $sformatf("Extra W beat on head, head_id=%0d exp=%0d got_beat=%0d WDATA=0x%0h WSTRB=0x%0h WLAST=%0b", int'(write_q[0].tr.id), expected_beats, beat_idx, vif.cb_monitor.wdata, vif.cb_monitor.wstrb, vif.cb_monitor.wlast))
                    end else begin
                        ...
                        `uvm_info("MON_W_HS",
                            $sformatf("W HS: head_id=%0d beat=%0d/%0d addr=0x%0h data=0x%0h wstrb=0x%0h wlast=%0b", int'(write_q[0].tr.id), beat_idx, expected_beats, beat_addr, vif.cb_monitor.wdata, vif.cb_monitor.wstrb, vif.cb_monitor.wlast), UVM_HIGH)
                        ...
                        if (vif.cb_monitor.wlast === 1'b1) begin
                            ...
                            if (write_q[0].beat_cnt != expected_beats) begin
                                `uvm_error("MON", $sformatf("WLAST mismatch, head_id=%0d seen_beats=%0d exp_beats=%0d", int'(write_q[0].tr.id), write_q[0].beat_cnt, expected_beats))
                            end
            ...
            if (bhs) begin
                ...
                if (wait_b_s.exists(id_k)) begin
                    if (!wait_b_s[id_k].b_seen) begin
                        ...
                        `uvm_error("MON", $sformatf("Duplicate B HS for BID=%0d, bresp=%0d", int'(bid_l), bresp_prev))
                    end
                    ...
                    if (idx < 0) begin
                        ...
                        if (in_ignore_window()) begin
                            state_lock.put(1);
                            `uvm_info("MON_B_DROP", $sformatf("Drop late/stray B during reset/flush window: BID=%0d bresp=%0d q_depth=%0d, ignore_until=%0t reset_active=%0b", int'(bid_l), bresp_prev, qd, ignore_unknown_until, reset_active), UVM_LOW)
                        end
                        else if (in_dup_b_window(id_k)) begin
                            time dt = $time - last_wr_done_time[id_k];
                            state_lock.put(1);
                            `uvm_info("MON_B_DUP_DROP", $sformatf("Drop DUP/late B after write completion: BID=%0d bresp=%0d dt=%0t q_depth=%0d", int'(bid_l), bresp_prev, dt, qd), UVM_LOW)
                        end
                        else begin
                            state_lock.put(1);
                            `uvm_error("MON", $sformatf("B HS for unknown BID=%0d, bresp=%0d q_depth=%0d", int'(bid_l), bresp_prev, qd))
                        end
                    end else begin
                        ...
                        if (write_q[idx].beat_cnt != expected_beats) begin
                            `uvm_info("MON_B_EARLY", $sformatf("B arrived early, BID=%0d beats=%0d/%0d", int'(bid_l), write_q[idx].beat_cnt, expected_beats), UVM_LOW)
                        end
        ...
        forever begin
            ...
            if ((vif.cb_monitor.arvalid === 1'b1) && (vif.cb_monitor.arready === 1'b0)) begin
                ...
                if ((ar_wait_cyc_r % 100) == 0) begin
                    `uvm_info("MON_AR_WAIT", $sformatf("AR waiting %0d cyc, v=%0b r=%0b addr=0x%0h id=%0d len=%0d burst=%02b size=%0d", ar_wait_cyc_r, vif.cb_monitor.arvalid, vif.cb_monitor.arready, vif.cb_monitor.araddr,  vif.cb_monitor.arid, vif.cb_monitor.arlen,   vif.cb_monitor.arburst, vif.cb_monitor.arsize), UVM_LOW)
                end
            end else begin
                ar_wait_cyc_r = 0;
            end
            ...
            if ((vif.cb_monitor.arvalid === 1'b1) && (vif.cb_monitor.arready === 1'b1)) begin
                ...
                // Build initialized entry
                new_e.tr = axi_mm_seq_item#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH)::type_id::create($sformatf("rd_tr_id_%0d", int'(vif.cb_monitor.arid)), this);
                ...
                if (pending_reads_s.exists(id_k)) begin
                    ...
                    `uvm_info("MON_AR_HS", $sformatf("AR HS: ID=%0d addr=0x%0h len=%0d burst=%02b size=%0d pending=%0d", int'(vif.cb_monitor.arid), vif.cb_monitor.araddr, vif.cb_monitor.arlen, vif.cb_monitor.arburst, vif.cb_monitor.arsize, pending_cnt_after), UVM_LOW)
                end
            end
            ...
            if ((vif.cb_monitor.rvalid === 1'b1) && (vif.cb_monitor.rready === 1'b0)) begin
                ...
                if ((r_wait_cyc_r % 100) == 0) begin
                    `uvm_info("MON_R_WAIT", $sformatf("R waiting %0d cyc, v=%0b r=%0b rid=%0d rlast=%0b rresp=%0b rdata=0x%0h", r_wait_cyc_r, vif.cb_monitor.rvalid, vif.cb_monitor.rready, int'(vif.cb_monitor.rid), vif.cb_monitor.rlast, vif.cb_monitor.rresp, vif.cb_monitor.rdata), UVM_LOW)
                end
            end else begin
                r_wait_cyc_r = 0;
            end
            ...
            if ((vif.cb_monitor.rvalid === 1'b1) && (vif.cb_monitor.rready === 1'b1)) begin
                ...
                if (!pending_reads_s.exists(id_k)) begin
                    if (in_ignore_window()) begin
                        state_lock.put(1);
                        `uvm_info("MON_R_DROP", $sformatf("Drop late/stray R during reset/flush window: RID=%0d rlast=%0b rresp=%0b rdata=0x%0h, ignore_until=%0t reset_active=%0b", int'(vif.cb_monitor.rid), vif.cb_monitor.rlast, vif.cb_monitor.rresp, vif.cb_monitor.rdata, ignore_unknown_until, reset_active), UVM_LOW)
                    end else begin
                        state_lock.put(1);
                    ...
                    if (beat_idx < expected_beats) begin
                        ...
                        `uvm_info("MON_R_HS", $sformatf("R HS: ID=%0d beat=%0d/%0d addr=0x%0h data=0x%0h rresp=%0b rlast=%0b", int'(vif.cb_monitor.rid), beat_idx, expected_beats, beat_addr, vif.cb_monitor.rdata, vif.cb_monitor.rresp, vif.cb_monitor.rlast), UVM_HIGH)
                    end else begin
                        `uvm_error("MON", $sformatf("Extra R beat: ID=%0d beat=%0d exp=%0d addr=0x%0h data=0x%0h rresp=%0b rlast=%0b", int'(vif.cb_monitor.rid), beat_idx, expected_beats, beat_addr, vif.cb_monitor.rdata, vif.cb_monitor.rresp, vif.cb_monitor.rlast))
                    end

                    if (vif.cb_monitor.rlast === 1'b1) begin
                        if (pending_reads_s[id_k].beat_cnt != expected_beats) begin
                            `uvm_error("MON", $sformatf("RLAST early/late, ID=%0d beats=%0d/%0d", int'(vif.cb_monitor.rid), pending_reads_s[id_k].beat_cnt, expected_beats))
                        end

                        // Stamp transactionn done_time at RLAST
                    ...
                    if (have_done) begin
                        `uvm_info("MON_RD_DONE", $sformatf("READ completed: ID=%0d addr=0x%0h beats=%0d done_time=%0t", int'(done_tr.id), done_tr.addr, (done_tr.len+1), done_tr.done_time), UVM_LOW)
                        ap.write(done_tr);
                    end
                end
            end
