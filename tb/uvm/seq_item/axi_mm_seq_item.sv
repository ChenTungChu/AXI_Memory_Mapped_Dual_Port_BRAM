...
typedef enum logic [2:0] {
    OP_FULL    = 3'd0,   // Write: AW+W+B, Read: AR+R
    OP_AW_ONLY = 3'd1,   // Only drive AW
    OP_W_ONLY  = 3'd2,   // Only drive W burst
...
class axi_mm_seq_item #(
    ...
    // op_kind consistent with Read/Write
    ...
    constraint c_addr_align {
        soft (
            ...
            (size == 7) ? (addr[6:0] == 7'b0) : 1);
    };

    // WRAP burst legality
    constraint c_wrap_legal {
        if (burst == 2'b10) {
            (((len + 1) & len) == 0);  // len+1 must be power of 2
            (
                (size == 0) ? 1 :
                (size == 1) ? (addr[0]   == 1'b0) :
                (size == 2) ? (addr[1:0] == 2'b00) :
    ...
    // For Write: Must have payload arrays sized to beats only when actually is driving W
        ...
        if (rw == AXI_READ) begin
            ...
            if (need_payload) begin
                foreach (wstrb_beats[i]) begin
                    // If random produced X or '0, force sane default '1
    ...
    // ------------------------------------------------------------
    // Helper function: Manual allocation
    // ------------------------------------------------------------
    ...
    virtual function string convert2string();
        return $sformatf("axi_mm_seq_item: %s op=%0d addr=0x%0h len=%0d size=%0d burst=%0b id=0x%0h wait_bid=0x%0h done=%0t", (rw==AXI_WRITE) ? "WRITE" : "READ", int'(op_kind), addr, len, size, burst, id, wait_bid, done_time);
    endfunction
