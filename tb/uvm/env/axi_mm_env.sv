...
class axi_mm_env #(
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    // Window bases
    // - config_db override
    // - Else fall back to `ifdef WIN0_BASE/WIN1_BASE
    // ------------------------------------------------------------
    ...
    `uvm_info("ENV", $sformatf("WIN bases: WIN0_BASE=0x%0h WIN1_BASE=0x%0h", win0_base, win1_base), UVM_LOW)
    ...
    // Commit monitor
    commit_mon = axi_mm_commit_monitor#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH, COMMIT_BEAT_IDX_W)::type_id::create("commit_mon", this);

    // Apply monitor
    apply_mon = axi_mm_apply_monitor#(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH, APPLY_BEAT_IDX_W)::type_id::create("apply_mon", this);
  ...
  virtual function void connect_phase(uvm_phase phase);
    ...
    if (scb.ap_imp_p0     == null ||
      ...
      `uvm_fatal("ENV", "Scoreboard analysis imps are null")
    ...
    // Commit monitor -> Scoreboard
    commit_mon.ap.connect(scb.ap_imp_commit);

    // Apply monitor -> Scoreboard
    ...
    `uvm_info("ENV", "Env connected: monitor -> scb, monitor -> cov_scoreboard, monitor -> cov_subscribers, commit_mon -> scb, apply_mon -> scb", UVM_LOW)
  endfunction
  ...
  task automatic do_initial_reset(
    ...
    if (rst_agent == null || rst_agent.seqr == null) begin
      `uvm_fatal("ENV_RST", "rst_agent/seqr is null")
    end
    ...
    `uvm_info("ENV_RST", $sformatf("Starting initial reset: assert_cycles=%0d deassert_cycles=%0d", rst_assert_cycles, rst_deassert_cycles), UVM_LOW)
    ...
    `uvm_info("ENV_RST", "Initial reset completed", UVM_LOW)
